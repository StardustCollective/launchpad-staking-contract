...
describe("LatticeStakingPool: disableProject", () => {
  let accounts: Signer[];
  ...
  it("Disables a project", async () => {
    ...
    // Assert Events
    expect(trxReceipt.events?.length).to.equal(1);
    expect(trxReceipt.events?.[0].event).to.equal("ProjectDisabled");
    expect(
      (
        trxReceipt.events?.[0] as ProjectDisabledEvent
      ).args._projectId.toNumber()
    ).to.equal(0);

    const block = await ethers.provider.getBlock(trxReceipt.blockHash);

    const project = await stakingContract.projects(0);
    expect(project.endTimestamp.toNumber()).to.equal(block.timestamp);
  });
  ...
  it("Rejects non-owner calls", async () => {
    try {
      ...
      if (e instanceof Error) {
        expect(e.message).to.match(/disableProject: Caller is not the owner/i);
      } else {
  ...
  it("Rejects invalid project ID", async () => {
    try {
      ...
      if (e instanceof Error) {
        expect(e.message).to.match(/disableProject: Invalid project ID/i);
      } else {
        throw new Error("Method should throw an Error");
      }
    }
