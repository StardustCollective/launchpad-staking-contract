...
contract("LatticeStakingPool: withdraw", (accounts) => {
  ...
  it("Withdraws funds", async () => {
    await timeTravel(DateTime.now().plus({ days: 10 }));

    const response = await withdrawFunds(0, 1, accounts[1]);

    const balance = await balanceLtx(accounts[1]);

    assert.equal(response.didUserWithdrawFunds, true);
    assert.equal(balance, 10000 * 1e8);

    // Assert Events
    assert.equal(response.trxResponse.logs.length, 1);
    assert.equal(response.trxResponse.logs[0].event, "Withdraw");
    assert.equal(
      (response.trxResponse.logs[0] as Truffle.TransactionLog<Withdraw>).args
        ._user,
      accounts[1]
    );
    assert.equal(
      (
        response.trxResponse.logs[0] as Truffle.TransactionLog<Withdraw>
  ...
  it("Rejects invalid project ID", async () => {
    try {
      const response = await stakingContract.withdraw(1, 0, {
        from: accounts[1],
      });
      throw new Error("Method should throw");
    } catch (e) {
      if (e instanceof Error) {
        assert.match(e.message, /withdraw: Invalid project ID/i);
      } else {
  ...
  it("Rejects invalid pool ID", async () => {
    try {
      const response = await stakingContract.withdraw(0, 3, {
        from: accounts[1],
      });
      throw new Error("Method should throw");
    } catch (e) {
      if (e instanceof Error) {
        assert.match(e.message, /withdraw: Invalid pool ID/i);
      } else {
        throw new Error("Method should throw an Error");
      }
    }
  ...
  it("Rejects invalid withdraw time (endTimestamp)", async () => {
    try {
      const response = await stakingContract.withdraw(0, 1, {
        from: accounts[1],
      });
      throw new Error("Method should throw");
    } catch (e) {
      if (e instanceof Error) {
        assert.match(e.message, /withdraw: Not yet permitted/i);
      } else {
        throw new Error("Method should throw an Error");
      }
    }
  ...
  it("Rejects user has no funds to withdraw", async () => {
    ...
    try {
      const response = await stakingContract.withdraw(0, 0, {
        from: accounts[1],
      });
      throw new Error("Method should throw");
    } catch (e) {
      if (e instanceof Error) {
        assert.match(e.message, /withdraw: No stake to withdraw/i);
      } else {
        throw new Error("Method should throw an Error");
      }
    }
