...
contract LatticeStakingPoolDev is ReentrancyGuard {
    ...
    function mutateProject(
        ...
        require(msg.sender == owner, "mutateProject: Caller is not the owner");

        projects[_projectId].name = _name;
        projects[_projectId].startTimestamp = _startTimestamp;
        projects[_projectId].endTimestamp = _endTimestamp;
    }

    function mutateStakingPool(uint256 _projectId, uint256 _poolId, uint256 _maxStakingAmountPerUser) public {
        require(
            msg.sender == owner,
            "mutateStakingPool: Caller is not the owner."
        );
        require(
            _projectId < projects.length,
            "mutateStakingPool: Invalid project ID."
        );

        stakingPoolInfo[_projectId][_poolId].maxStakingAmountPerUser = _maxStakingAmountPerUser;
    }
}
